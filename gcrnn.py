...
class GCGRUCell(nn.Module):
    ...
    def forward(self, x, h, adj, m_in):
        x_gates = torch.cat([x, h], dim=1)
        r = torch.sigmoid(self.forget_gate(x_gates, adj, m_in))
        u = torch.sigmoid(self.update_gate(x_gates, adj, m_in))
        x_c = torch.cat([x, r * h], dim=1)
        c = self.c_gate(x_c, adj, m_in)  # batch_size, self._num_nodes * output_size
