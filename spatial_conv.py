...
class SpatialConvOrderK(nn.Module):
    def __init__(self, c_in, c_out, support_len=4, order=2, include_self=True):
    ...
    def compute_support(adj, device=None):
        if device is not None:
            adj = adj.to(device)
        adj_bwd = adj.T
        adj_fwd = adj / (adj.sum(1, keepdims=True) + 1e-5)
        adj_bwd = adj_bwd / (adj_bwd.sum(1, keepdims=True) + 1e-5)
    ...
    def forward(self, x, support, mask):
        ...
        # renormal [batch, nodes, nodes, steps]
        # mask [batch , nodes, nodes, steps] 
        if x.dim() < 4:
            squeeze = True
